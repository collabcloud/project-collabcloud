  ...
  async (req, res) => {
    try {
      ...
      if (!userIsInProject) {
        return res
          .status(404)
          .json({ result: "Unsuccessful. User not part of that project" });
      }

      // The project owner cannot leave the project
      if (
        req.body.memberStatus == "owner" ||
        userIsInProject.dataValues.isOwner
      ) {
        return res
          .status(400)
          .json({ errors: "The owner may not leave the project" });
      }
      ...
      if (record) {
        ...
        if (success) {
          ...
          notificationHelpers.addNotification(
            ...
            req.body.uid,
            project.ownerId,
            `${username} left ${project.projectName} at ${moment().format(
