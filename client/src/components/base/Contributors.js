...
export function Contributors(props) {
  ...
  collaborators.forEach( collaborator => {
    ...
    cleanedCollaborator.joinDate = moment(collaborator.createdAt).format("MMMM Do YYYY, h:mm:ss a");
    let currentDate = moment().startOf('day');
    let createdDate = moment(collaborator.createdAt).format("YYYY-MM-DD");
    cleanedCollaborator.daysOnProject = moment.duration(currentDate.diff(createdDate)).asDays() + 1;
  ...
  return (
    <Jumbotron>
      ...
      >
        <TableHeaderColumn width="15%" dataSort dataField="role"><GoInfo/> Role</TableHeaderColumn>
        <TableHeaderColumn width="40%" isKey dataSort dataField="username"><GoPerson/> Username </TableHeaderColumn>
        <TableHeaderColumn width="30%" dataSort dataField="joinDate"><GoCalendar/> Joined</TableHeaderColumn>
        <TableHeaderColumn width="15%" dataSort dataField="daysOnProject"><GoClock/> Days on Project</TableHeaderColumn>
      </BootstrapTable>
    </Jumbotron>);
}